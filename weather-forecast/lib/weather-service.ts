...
export async function fetchWeatherData(location: Location): Promise<WeatherData> {
  try {
    // Use same-origin API route so Railway/production can route via Next rewrites.
    const response = await fetch(`/api/weather/forecast/${encodeURIComponent(location.name)}`);
